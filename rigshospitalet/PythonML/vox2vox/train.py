...
train_dataset = NIFTIDataset(args.traincsv, args.fullDose, args.lowDose,
                            input_size=args.input_size, 
                            depth_size=args.depth_size, 
                            augment=None)

val_dataset = NIFTIDataset(args.valcsv, args.fullDose, args.lowDose,
                            input_size=args.input_size, 
                            depth_size=args.depth_size, 
                            augment=None)
...
if args.resume_weight == "":
    modelname = f"{datetime.datetime.now().strftime('%y-%m-%dT%H%M%S')}-glr_{args.glr}-dlr_{args.dlr}-bs_{args.batchsize}_sizes{args.input_size}x{args.input_size}x{args.depth_size}_gradac_{args.gradientAccumSteps}"
...
if args.resume_weight:
    ...
    trainer.opt_G.param_groups[0]["lr"] = args.glr
    trainer.opt_D.param_groups[0]["lr"] = args.dlr

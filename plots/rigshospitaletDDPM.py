...
# W&B settings
ENTITY = ""
PROJECT = "medDDPMRigshospitalet"

api = wandb.Api()
runs = api.runs(f"{ENTITY}/{PROJECT}")

# Metrics to keep
selected_columns = ["step", "validation_loss", "training_loss", "SSIM", "PSNR", "val_noise_pred_loss", "time_elapsed (min)", "learning_rate"]
...
for run in runs:
    ...
    history["epoch"] = (history["step"] // 1195).astype(int)
...
if all_metrics:
    ...
    metrics_to_plot = [ "training_loss", "val_noise_pred_loss", "SSIM", "PSNR", "validation_loss", "cumulative_time (hours)", "learning_rate"]
    
    # Create a plot for each run
    for run_name in df_agg["run_name"].unique():
        fig, axes = plt.subplots(4, 2, figsize=(20, 10))  # 3 rows, 2 columns of subplots
        ...
        plt.tight_layout()
        plt.savefig(f"rigshospitaletDDPM/{run_name}_metrics.png")
        plt.close()

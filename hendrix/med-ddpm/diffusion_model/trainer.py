...
class Trainer(object):
    def __init__(
        ...
        wandb_project="medDDPM",  # Wandb project name
        val_batches = 3,
        model_name = "model",
        lr_decay_step=5000,
        lr_decay_factor=0.1,
        ...
        self.ds = dataset
        self.dl = cycle(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=True, num_workers=3, pin_memory=True))

        self.val_dataset = val_dataset
        self.val_dl = None
        if val_dataset is not None:
            self.val_dl = cycle(torch.utils.data.DataLoader(val_dataset, batch_size=train_batch_size, shuffle=True, num_workers=3, pin_memory=True))
        ...
        with torch.no_grad():
            ...
            for _ in range(self.val_batches): 
                ...
                if self.with_condition:
                    ...
                    generated = self.ema_model.sample(batch_size=input_tensors.shape[0], condition_tensors=input_tensors)
                else:
                    data = data.cuda()
                    generated = self.ema_model.sample(batch_size=input_tensors.shape[0], condition_tensors= None)
    ...
    def validate_noise_prediction(self):
        ...
        with torch.no_grad():
            for _ in range(num_batches):
                ...
                t = torch.randint(0, self.model.num_timesteps, (b,), device=target_tensors.device).long()
                
                # Get noise prediction loss
                if self.with_condition:
                    noise_pred_loss = self.model.p_losses(
    ...
    def adjust_learning_rate(self):
        if self.step >= self.lr_decay_step:
            new_lr = self.initial_lr * self.lr_decay_factor
            for param_group in self.opt.param_groups:
                param_group['lr'] = new_lr
            print(f'Learning rate adjusted to {new_lr} at step {self.step}')
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                if self.val_dl is not None:
                    ...
                    with torch.no_grad():
                        ...
                        generated = self.ema_model.sample(batch_size=input_tensors.shape[0], condition_tensors=input_tensors if self.with_condition else None)
                    ...
                    if self.use_wandb:
                        ...
                        wandb.log({
                            "sample_image": wandb.Image(slice_2d, caption=f"Step {self.step}"),
                        })

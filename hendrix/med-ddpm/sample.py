...
test_dataset = DicomDataset(testcsv, targetfolder, input_size=input_size, depth_size=depth_size, augment=None)
...
with torch.no_grad():
    for i, data in enumerate(test_dataset):
        ...
        # Save as NIfTI files
        nib.save(
            nib.Nifti1Image(generated_hu.copy(), affine=np.eye(4)), 
            os.path.join(exportfolder, f'testimgGenerated-{num_img}.nii.gz')
        )
        nib.save(
            nib.Nifti1Image(input_hu.copy(), affine=np.eye(4)), 
            os.path.join(exportfolder, f'testimgInput-{num_img}.nii.gz')
        )
        nib.save(
            nib.Nifti1Image(target_hu.copy(), affine=np.eye(4)), 
            os.path.join(exportfolder, f'testimgTarget-{num_img}.nii.gz')
        )
        ...
        nib.save(
            nib.Nifti1Image(error_target_generated, affine=np.eye(4)),
            os.path.join(exportfolder, f'errorTargetGenerated-{num_img}.nii.gz')
        )
        nib.save(
            nib.Nifti1Image(error_target_input, affine=np.eye(4)),
            os.path.join(exportfolder, f'errorTargetInput-{num_img}.nii.gz')
        )

...
def analyze_segmentations(base_folder, num_samples=15, fast=True):
    ...
    target_path = os.path.join(base_folder, f'testimgTarget-1.nii.gz')
    ...
    for i in range(1, num_samples + 1):
        ...
        generated_path = os.path.join(base_folder, f'testimgGenerated-{i}.nii.gz')
        input_path = os.path.join(base_folder, f'testimgInput-{i}.nii.gz')
        target_path = os.path.join(base_folder, f'testimgTarget-{i}.nii.gz')
...
if __name__ == "__main__":
    ...
    results = analyze_segmentations(base_folder, 15, fast=False)
